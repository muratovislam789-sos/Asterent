...
export const chatRepository = {
  async findOrCreate(listingId: string, tenantId: string, landlordId: string) {
    const existing = await pool.query(
      'SELECT * FROM chats WHERE listing_id=$1 AND tenant_id=$2 AND landlord_id=$3',
      [listingId, tenantId, landlordId]
    )
    if (existing.rows[0]) return this.formatChat(existing.rows[0])
    const { rows } = await pool.query(
      'INSERT INTO chats (listing_id, tenant_id, landlord_id) VALUES ($1,$2,$3) RETURNING *',
      [listingId, tenantId, landlordId]
    )
  ...
  async saveMessage(chatId: string, senderId: string, text: string) {
    const { rows } = await pool.query(
  ...
  async getMessages(chatId: string, page = 1, limit = 50) {
    ...
    const { rows } = await pool.query(
      'SELECT * FROM messages WHERE chat_id = $1 ORDER BY created_at ASC LIMIT $2 OFFSET $3',
      [chatId, limit, offset]
    )
  ...
  async markAsRead(chatId: string, userId: string) {
    await pool.query('UPDATE messages SET is_read = true WHERE chat_id = $1 AND sender_id != $2', [chatId, userId])
  },
